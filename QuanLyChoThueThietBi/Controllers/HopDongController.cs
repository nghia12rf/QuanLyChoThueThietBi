...
namespace RentalEquipmentAPI.Controllers
{
    ...
    public class HopDongController : ControllerBase
    {
        ...
        [HttpGet("CuaToi")]
        public async Task<ActionResult<IEnumerable<HopDongDto>>> GetHopDongsCuaToi([FromQuery] string? trangThai)
        {
            try
            {
                ...
                if (userIdClaim == null)
                {
                    return Unauthorized(new { message = "Bạn cần đăng nhập lại!" });
                }
                ...
                var query = _context.HopDongs
                    ...
                    .Include(h => h.ChiTietHopDongs)
                        .ThenInclude(ct => ct.MaThietBiNavigation)
                    .Where(h => h.MaNguoiTao == currentUserId)
                    .AsSplitQuery()
                    .AsQueryable();
            ...
            catch (Exception ex)
            {
                ...
                {
                    message = "Lỗi truy vấn hợp đồng của nhân viên",
                    detail = ex.Message
                });
            }
        }
